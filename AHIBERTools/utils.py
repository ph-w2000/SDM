...
def visualize(data_item, out=None):
    ...
    if hor.shape[-1] == 2:
        hor = hor.transpose((2, 0, 1))
        hor = np.ascontiguousarray(hor)

    if ver.shape[-1] == 2:
        ver = ver.transpose((2, 0, 1))
    ...
    new_3d = pose3d.copy()
    new_3d[:,:,0] = (new_3d[:,:,0] + 2)/4
    new_3d[:,:,1] = (new_3d[:,:,1] - 2)/4
    new_3d[:,:,2] = (new_3d[:,:,2] + 8)/4

    new_2d = np.concatenate((new_3d[:,:,0:1]*1640,1248-new_3d[:,:,2:]*1248),-1)
    # print(new_2d)
    pose2d = new_2d
